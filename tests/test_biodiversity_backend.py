...
def test_limited_seasonal_bird_and_date_controls_month() -> None:
    ...
    assert result.bundle.candidate_sites == []
    assert result.bundle.contextual_sites
    assert all(
        site.evidence_tier == SiteEvidenceTier.ungrounded
        for site in result.bundle.contextual_sites
    )
    assert result.plan.suggested_actions == [
...
def test_ambiguous_bird_requires_selection() -> None:
    ...
    assert result.bundle.candidate_sites == []
    assert result.plan.suggested_actions == [SiteSearchAction.select_taxon]
...
def test_invalid_bird_name() -> None:
    ...
    assert result.plan.suggested_actions == [SiteSearchAction.correct_bird_input]
...
def test_walking_limit_survives_in_structured_plan() -> None:
    ...
    routing = [
        item
        for item in result.plan.unresolved_constraints
        if item.code == "routing_not_available"
    ]
...
def test_scientific_and_common_fixture_paths_are_deterministic(
    bird: str, month: int
) -> None:
