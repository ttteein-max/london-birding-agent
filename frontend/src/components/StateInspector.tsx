    ...
    <aside className="drawer state-drawer" aria-labelledby="state-title" aria-live="polite">
      ...
      {loading || !state ? <p className="empty-copy">Loading allow-listed checkpoint state…</p> : (
        <div className="inspector-content">
          ...
          <section><h3>Human decisions & counters</h3><dl><Row label="Waiting for input" value={state.hitl.waiting} /><Row label="Interrupt" value={state.hitl.interrupt_kind} /><Row label="Evidence rounds" value={state.counters.evidence_loop_count} /><Row label="Plan revisions" value={state.counters.plan_revision_count} /><Row label="Recorded tools" value={state.counters.recorded_tool_call_count} /></dl></section>
        </div>
      )}
    </aside>
  );
