...
Phase 5 uses workflow `phase-5.0`, state schema `3` and route schema `1`. Every new `RunManifest` records data/model mode, model and endpoint fingerprints, routing provider and version, `foot-walking` profile, route schema and routing endpoint fingerprint. Resume, replay and fork validate exact thread, checkpoint, branch and execution identity plus current runtime compatibility.
